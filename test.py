...
class MomentumScaffoldingTests(unittest.TestCase):
    def test_proto_exposes_momentum_fields(self):
        proto_text = (THIS_DIR / "proto" / "cormoran" / "rip" / "custom.proto").read_text()
        self.assertIn("bool momentum_enabled", proto_text)
        self.assertIn("uint32 momentum_decay_ms", proto_text)
        self.assertIn("uint32 momentum_min_velocity", proto_text)
        self.assertIn("SetMomentumEnabledRequest", proto_text)
        self.assertIn("SetMomentumDecayRequest", proto_text)
        self.assertIn("SetMomentumMinVelocityRequest", proto_text)

    def test_runtime_header_exposes_momentum_config(self):
        header_text = (THIS_DIR / "include" / "zmk" / "pointing" / "input_processor_runtime.h").read_text()
        self.assertIn("bool momentum_enabled;", header_text)
        self.assertIn("uint16_t momentum_decay_ms;", header_text)
        self.assertIn("uint16_t momentum_min_velocity;", header_text)
        self.assertIn("zmk_input_processor_runtime_set_momentum", header_text)

    def test_readme_mentions_momentum_scrolling(self):
